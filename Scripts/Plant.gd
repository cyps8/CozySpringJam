...
@export var matureOffset: float = 0.0
...
func Planted():
	...
	tween.tween_property(plantSprite, "scale", Vector2(0.4, 0.4), 3.0).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_SINE)

func GrowToStage(stage: int):
	if stage <= stages.size() - 1:
		plantSprite.texture = stages[stage]
		var tween: Tween = create_tween()
		tween.tween_property(plantSprite, "scale", Vector2(0.4 + (0.3 * stage), 0.4 + (0.3 * stage)), 3.0).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_SINE)
	# else:
	# 	Blossom()
	if stage == 2:
		position.y += matureOffset
